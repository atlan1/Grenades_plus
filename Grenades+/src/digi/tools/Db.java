...
public class Db
{
	...
	/**
	 * Get a boolean from query(), automatically checks for null.
	 * 
	 * @param result the returned value of query()
	 * @param column the column number, starts from 1
	 * @return boolean value of the column or null
	 */
	public Boolean resultBoolean(ResultSet result, int column)
	{
		if(result == null)
			return null;
		...
		{
			result.next();
			Boolean bool = result.getBoolean(column);
			result.close();
